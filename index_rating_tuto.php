<?php
    ...
    if (isset($_POST['save'])) {
        ...
        $ratedIndex = $db->real_escape_string($_POST['ratedIndex']);
        $ratedIndex++;

        if (!$uID) {
            $db->query("INSERT INTO notes (rateIndex) VALUES ('$ratedIndex')");
            $sql = $db->query("SELECT id FROM stars ORDER BY id DESC LIMIT 1");
            $uData = $sql->fetch_assoc();
            $uID = $uData['id'];
        } else
            $db->query("UPDATE stars SET rateIndex='$ratedIndex' WHERE id='$uID'");
